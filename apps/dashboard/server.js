...
const TOKENS_CACHE_FILE = path.join(DATA_DIR, 'tokens.json');
...
// ─── JSONL parsing ────────────────────────────────────────────────────────────
// Each line in a session JSONL may be an "assistant" entry with usage metadata.
...
// Collect all JSONL paths inside a project hash directory.
// Structure:
//   {project-dir}/{session-uuid}.jsonl          — main session
//   {project-dir}/{session-uuid}/subagents/*.jsonl — sub-agent sessions
...
async function aggregateTokens() {
  ...
  // Current month cost (from day 1 of current month to today)
  const now = new Date();
...
// In-memory cache: avoid re-scanning the entire JSONL tree on every request
let tokenCache     = null;
...
async function getTokenData() {
  ...
  try {
    fs.writeFileSync(TOKENS_CACHE_FILE, JSON.stringify(tokenCache, null, 2));
  } catch { /* non-critical */ }
...
async function getCodespaceData() {
  ...
  try {
    ...
    // Billing endpoint exists for some account types; skip gracefully if not
    let billing = null;
    try {
...
app.listen(PORT, () => {
  ...
  console.log(`GITHUB_TOKEN: ${process.env.GITHUB_TOKEN ? 'configured' : 'NOT SET'}`);
});
